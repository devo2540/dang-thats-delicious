...
function autocomplete(input, latInput, lngInput) {
    ...
    var dropdown = new google.maps.places.Autocomplete(input); // get autocomple dropdown from google API

    dropdown.addListener('place_changed', function () {
        var place = dropdown.getPlace(); // get place from address input
...
function typeAhead(search) {
    ...
    searchInput.on('input', function () {
        ...
        _axios2.default.get('/api/search?q=' + this.value).then(function (res) {
            if (res.data.length) {
                searchResults.innerHTML = _dompurify2.default.sanitize(searchResultsHTML(res.data)); // display search results
                return;
            }
            // or tell user nothing came back
            searchResults.innerHTML = _dompurify2.default.sanitize('\n                    <div class="search__result"> No results for <strong> ' + _this.value + ' </strong> found! </div>\n                ');
        }).catch(function (err) {
            console.error(err);
        });
    });
